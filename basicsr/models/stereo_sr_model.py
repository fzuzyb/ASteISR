...
class StereoSRModel(BaseModel):
    ...
    def setup_optimizers(self):
        ...
        if train_opt.get('use_adapter_train',False):
            ## freeze some parameters
            for name, param in self.net_g.named_parameters():
                param.requires_grad = False
                requires_grad = False
                for need_opt_param in self.opt['network_g']['opt_params']:
                    if need_opt_param in name:
                        requires_grad = True
                        break
        ...
        num_total_param = sum(p.numel() for p in self.net_g.parameters())
    ...
    def optimize_parameters(self, current_iter):
        ...
        if self.opt['train'].get('use_adapter_train',False):
            l_total = l_total + 0. * sum(p.sum() for p in self.net_g.parameters())
        # else:
        #     l_total = l_total + 0. * sum(p.sum() for p in self.net_g.parameters() if p.requires_grad)
        l_total.backward()
    ...
    def padding_for_aug(self,img):
        window_size = self.opt['network_g'].get('window_size', (0, 0))
        ...
        if window_size[0] > 0 and window_size[1] > 0:
            ...
            if h % window_size[0] != 0:
                mod_pad_h = window_size[0] - h % window_size[0]
            if w % window_size[1] != 0:
                mod_pad_w = window_size[1] - w % window_size[1]
            img = F.pad(img, (0, mod_pad_w, 0, mod_pad_h), 'reflect')
        else:

            mod_pad_w = 0
            mod_pad_h = 0
        return img,mod_pad_w,mod_pad_h
    def unpadding_for_aug(self,img,mod_pad_w,mod_pad_h):
        scale = self.opt.get('scale', 1)
        _, _, h, w = img.size()
        if mod_pad_w != 0 or mod_pad_h != 0:
    ...
    def pre_process(self):
        ...
        if window_size[0]>0 and window_size[1] >0:
            ...
            if h % window_size[0] != 0:
                self.mod_pad_h = window_size[0] - h % window_size[0]
            if w % window_size[1] != 0:
                self.mod_pad_w = window_size[1] - w % window_size[1]
    ...
    def post_process(self):
        ...
        if self.mod_pad_w!=0 or self.mod_pad_h!=0:
        ...
        if hasattr(self, 'net_g_ema'):
            ...
            with torch.no_grad():
                if self.img.size()[0] == 1:
                    ...
                    for lq in self.img:
                        outs.append(self.net_g_ema(lq.unsqueeze(0)).cpu())
            ...
            with torch.no_grad():
                if self.img.size()[0] == 1:
                    ...
                    for lq in self.img:
                        outs.append(self.net_g(lq.unsqueeze(0)).cpu())
    ...
    def grids_inverse(self):
        self.output = torch.split(self.output,split_size_or_sections=1,dim=0)
        ...
        for cnt, each_idx in enumerate(self.idxes):
            i = each_idx['i']
            j = each_idx['j']
            preds[0, :, i: i + crop_size_h, j: j + crop_size_w] += self.output[cnt].squeeze(0)
        ...
        if hasattr(self, 'net_g_ema'):
            ...
            with torch.no_grad():
                for aug in lq_list:
                    ...
                    aug, padw, padh = self.padding_for_aug(aug)
                    aug = self.net_g_ema(aug)
                    aug = self.unpadding_for_aug(aug, padw, padh)
                    out_list.append(aug)
